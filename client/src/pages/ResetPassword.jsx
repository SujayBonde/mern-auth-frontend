...
const ResetPassword = () => {
  ...
  const onSubmitOtp = async (e) => {
    e.preventDefault();
    const otpArray = Array.isArray(inputRefs.current)
    ...
    <>
      ...
      <form onSubmit={onSubmitOtp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='flex justify-between mb-8'>
          {Array(6).fill(0).map((_, index)=>(
            ...
            onPaste={handlePaste} type="text" maxLength='1' key={index} required/>
          ))}
        </div>
      ...
      <form onSubmit={onSubmitNewPassword} className="bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm">
        ...
        <div className="mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]">
          ...
          <input className="bg-transparent outline-none text-white" type="password" placeholder="New Password" value={newPassword} onChange={e =>setNewPassword(e.target.value)}/>
        </div>
        <button className="w-full py-3 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full cursor-pointer">Submit</button>
      </form> 
      }
